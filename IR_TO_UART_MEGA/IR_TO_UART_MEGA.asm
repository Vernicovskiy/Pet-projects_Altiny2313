...
ДЛЯ ATmega16:	Вывод захвата - PD6, (20 вывод)		Вывод передачи TXD USART PD1 (15 вывод)
ДЛЯ ATmega8:	Вывод захвата - PB0, (14 вывод)		Вывод передачи TXD USART PD1 (3 вывод)
...
Максимальное число измеряемых интервалов (длительностей 1 или 0)	198 (можно и больше, но нумерация строк после 99 нарушается)

КОНТРОЛЛЕР:			ATmega16, ATmega8
ТАКТОВАЯ ЧАСТОТА:	8 MHz
 */ 
.include "m16def.inc"				;Для микроконтроллера ATmega16
;.include "m8def.inc"				;Для микроконтроллера ATmega8

.equ		Numbers_of_commands	=198			;Число длительностей нулей и единиц, которые могут быть приняты
...
INIT:			outi	SPH,high(RAMEND)
				...
				outi	TIMSK,(1<<TOIE1|1<<TICIE1)	;ПРЕРЫВАНИЕ ПО ПЕРЕПОЛНЕНИЮ, ПРЕРЫВАНИЕ ПО ЗАХВАТУ
